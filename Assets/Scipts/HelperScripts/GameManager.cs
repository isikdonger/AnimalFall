...
public class GameManager : MonoBehaviour
{
    ...
    {
        GooglePlayServicesManager.IsAchievementUnlocked("Polandball Cannot Into Space", isUnlocked =>
        {
            if (!isUnlocked)
            {
                if (ScoreTextScript.scoreValue == 1000)
                {
                    GooglePlayServicesManager.UnlockAchievementCoroutine("Polandball Cannot Into Space");
                }
            }
        });
    }
